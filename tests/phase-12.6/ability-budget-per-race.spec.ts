 ...
 *    has a snapshot entry AND that a canonical baseline {8,8,8,8,8,8}
 *    yields remainingPoints === budget AND that bumping DEX 8→14 spends
 *    exactly `costByScore['14']-costByScore['8']` points.
 ...
 *    future-work pin; it is NOT a silent scope reduction — the provenance
 *    doc at `packages/rules-engine/src/foundation/data/puerta-point-buy.md`
 *    captures the evidence and the condition under which variance would
 *    surface (server-script overrides that Phase 05.1 client extraction
 *    cannot observe).
 */
...
// Cast at the dedupe boundary so `describe.each` and the snapshot lookup
// are well-typed for the populated Plan 06 snapshot.
const uniqueRaces = [
  ...new Set(compiledRaceCatalog.races.map((r) => r.id)),
] as CanonicalId[];

describe.each(uniqueRaces)(
  'Phase 12.6 — per-race ability budget — %s (SPEC R4, Plan 06)',
  (raceId: CanonicalId) => {
...
describe('Phase 12.6 — per-race variance note (SPEC R2+R4, Plan 06 variance-gate override)', () => {
  it('Elfo and Enano at identical baseAttributes currently produce identical remainingPoints because sourced curve is uniform (racialtypes.2da AbilitiesPointBuyNumber=30 + NWN1 hardcoded cost step)', () => {
    ...
    // so uniformity IS the truthful state. This assertion pins that reality
    // so future data changes are caught: if a server-script override lands
    // and Elfo/Enano start producing different totals, THIS test flips red
    // and the per-race variance story reopens.
