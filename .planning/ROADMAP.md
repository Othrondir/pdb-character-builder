...
- [x] 17-02-PLAN.md — Rules-engine + selector: NWN1_POINT_BUY_COST_TABLE + deriveAbilityBudgetRules helper + AbilityBudgetRules export promotion + selectAbilityBudgetRulesForRace rewire + Wave 2 helper + selector RED-gate specs (SC#4 D-03 reframe). _Done 2026-04-28 — `b368509` (helper RED) + `f048c9f` (helper GREEN) + `74e76cb` (selector RED) + `4e24102` (selector GREEN). 1 spec migration brought forward from Wave 3 (Rule 3 deviation: phase-12.6 attributes-board-fail-closed seed mechanism)._
- [x] 17-03-PLAN.md — Atomic snapshot retirement: 4 importer specs migrated (1 already in W2) + `point-buy-snapshot-coverage.spec.ts` deleted + snapshot module + JSON + provenance dossier deleted + foundation barrel cleaned (5 exports, was 6) + UAT-FINDINGS-2026-04-20 §A1 closeout footer with verbatim D-05 disposition note + git-history evidence pointer. _Done 2026-04-28 — `bed5239` (test phase-12.6 migration) + `b4fdac0` (test pre-12.6 seeder migration) + `b1539fd` (refactor atomic retirement). 2 Rule-1 deviations auto-fixed (zero-grep documentation hygiene). Full grep sweep across `apps/ packages/ tests/` for all 4 banned tokens → ZERO matches. Phase 17 ready for `/gsd-verify-work 17`._
