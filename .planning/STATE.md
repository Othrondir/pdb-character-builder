...
- Phase 12.6 inserted after Phase 12.4: UAT-2026-04-20 Residuals (INSERTED) — User dictated 8 findings 2026-04-20; hot-fix batch closed P1 + G1 + G2 + A2 + P2 + P3 + P4 + P6 (commits `e8181a5..9513fe6` on master, 632/632 green). **A1** (point-buy cost per race — Puerta assigns different cost curves per race, current `ability-budget.ts` uses uniform curve; blocked on source data: extractor enrichment or Puerta snapshot override) and **P5** (level table / progression usability redesign — open-ended) remain. Skipped decimal 12.5 because the hot-fix batch was not formally tracked as a phase directory. Next: `/gsd-spec-phase 12.6`.
...
Last session: 2026-04-20T21:12:26Z
Stopped at: **Phase 12.7-01 COMPLETE — F7 BLOCKER closed at the code level.** Wave 1 unblock plan (12.7-01 / 12.7-04 are parallel candidates; 12.7-01 shipped first). Hoisted <LevelEditorActionBar /> out of <LevelProgressionRow />'s expanded slot into <CreationStepper />. D-01 mount site + D-02 stable key=expandedLevel + D-03 zero-occurrence deletion of old mount + D-04 testid-contract-preserved all honored verbatim. Auto-fixed Rule 3: Phase 12.4-09 E2E spec harness narrowed to sibling-mount LevelEditorActionBar + BuildProgressionBoard (full CreationStepper OOM'd the Vitest worker); Phase 12.6 scan spec C1/D1 assertions flipped to match the new row-contract (action bar + advance button no longer inside the expanded row). Commits d5d7246 (RED: 6 suites, 5 failing + A4 L20-null passing) + d38ee18 (GREEN: hoist + old-mount delete + cross-spec harness updates). Full suite 797 passed / 2 skipped / 1 todo across 97 test files (zero failures, zero regressions vs Phase 12.6 close baseline of 632/632). Typecheck clean.

Resume options next session:

1. **Execute Phase 12.7-02** — R2 skill over-allocation gate (`canIncrementSkill` helper in `packages/rules-engine/src/skills/skill-budget.ts` + wire at `skill-sheet.tsx:77` disabled prop). Wave 2 after 12.7-01 (which just completed). Plan file: `.planning/phases/12.7-uat-04-20-post-12.6-residuals/12.7-02-PLAN.md`.
2. **Execute Phase 12.7-04** — R6 L2..L20 skill-budget formula verify-only spike. Independent of 12.7-01/02; can run in parallel with Wave 2. If green: no-op summary; if red: document divergences and defer fix to Phase 12.8.
3. **Execute Phase 12.7-03** — R3 (Habilidades scroll reset) + R4 (drop per-row Clase/Transclase) + R5 (Nivel N row prefix) UI polish. Wave 3, depends on 12.7-02.
4. **Human UAT spot-check of 12.7-01** — navigate Atributos → Aceptar → Progresion-Habilidades / Dotes on live dev server; confirm action bar is visible on each sub-step and that the DOM node does not flicker / remount when changing sub-step within the same level.

Resume file: `.planning/phases/12.7-uat-04-20-post-12.6-residuals/12.7-01-SUMMARY.md`
