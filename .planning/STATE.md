...
- [Phase 12.4-03]: per-level-budget selector in packages/rules-engine/src/progression/per-level-budget.ts composes determineFeatSlots + evaluateSkillLevel-style formula; Humano L1 bonus feat + Humano +1 SP/level hardcoded per A1/A2 with race-enrichment TODO. Framework purity preserved via INLINE structural input types (ClassCatalogInput / FeatCatalogInput / RaceCatalogInput) — zero imports from @data-extractor (grep returns 0 on both `@data-extractor` and `data-extractor` substrings). OQ-3 resolved: CLASS_BONUS_FEAT_SCHEDULES extended with class:wizard [1,5,10,15] + class:monk [1,2,6] + class:rogue [10,13,16] (RED surfaced 10 failures across Mago/Pícaro/Monje/Humano+Mago; one 6-line schedule edit closed all ten). A2 arithmetic locked: L1 Humano = (base × 4) + 4, L>1 Humano = base + 1. INT-increase arithmetic locked via Pícaro L12 fixture case (INT 14 + 2 bumps → mod 3 → 11 SP). CLASS_SKILL_POINTS duplicate in skills/selectors.ts deferred to 12.4-03.1 follow-up. Bench p99 0.0046 ms at L16 (SPEC R3 p95 < 1 ms gate passes with ~200× headroom). 526/527 full suite (+28 new, 0 regressions; 1 pre-existing theme-contract failure unrelated to this plan).
