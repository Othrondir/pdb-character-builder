...
- [Phase 12.4-05]: Habilidades board renders two <section> blocks (Habilidades de clase · coste 1 pt/rango / Habilidades transclase · coste 2 pts/rango). Selector groups rows by costType (class/cross-class) instead of category. CSS namespace .skill-board__section-heading + .skill-board__cost-hint added to app.css; cost hint font-size locked ≥12px via scoped CSS + inline belt-and-braces style (jsdom CSS gap workaround). Solo entrenada badge remains orthogonal per-row (now exposes data-trained-only='true' for regression queries). Closes SPEC R4, SKIL-01, SKIL-02.
